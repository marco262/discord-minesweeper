...
def print_task_list(db, owner: Message.author, print_nums=True):
    task_list = db.get_tasks(get_list_items(db, owner))
    output = []
    for i, task in enumerate(task_list):
        output.append(f"{get_icon(task)} {task['name']}" + (f"    ({i + 1})" if print_nums else ""))
    return "\n".join(output)


def get_list_items(db, owner: Message.author):
    task_list = db.get_list_items(owner.id)
    if task_list is None:
        raise ListBotError(f"I don't have a list assigned to {owner.name}. "
